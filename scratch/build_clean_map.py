# Python script to write clean, vibrant AntarcticMap.tsx without dark shading, without SIC, with all circum-Antarctic H3 cells and individual route toggles.
...
import antarcticaFullH3Data from '../data/antarctica_full_h3.json';
...
export const AntarcticMap: React.FC<AntarcticMapProps> = ({
  ...
  // 1. Full Circum-Antarctic H3 Grid (3,848 cells covering entire Southern Ocean)
  const fullAntarcticaH3GeoJSON: GeoJSON.FeatureCollection = useMemo(() => {
    return antarcticaFullH3Data as GeoJSON.FeatureCollection;
  }, []);
  ...
  // -------------------------------------------------------------
  // Map Initialization: Natural, Vibrant Basemap & Full H3 Grid
  // -------------------------------------------------------------
  useEffect(() => {
    ...
    mapInstance.on('load', () => {
      ...
      // 1. Natural Basemap (already in style, full opacity)
      // 2. Full Circum-Antarctic H3 Hexagonal Grid (3,848 cells)
      ...
      // 2. FULL CIRCUM-ANTARCTIC H3 GRID
      mapInstance.addSource('full-h3-source', {
        type: 'geojson',
        data: fullAntarcticaH3GeoJSON
      });

      // Subtle, elegant hexagonal mesh lines across the entire Antarctica ocean
      mapInstance.addLayer({
        id: 'full-h3-lines',
        type: 'line',
        source: 'full-h3-source',
        layout: {
        ...
        paint: {
          'line-color': [
            ...
            ['linear'],
            ['zoom'],
            2, 'rgba(56, 189, 248, 0.35)',
            5, 'rgba(56, 189, 248, 0.70)'
          ],
          'line-width': [
            'interpolate',
            ['linear'],
      ...
      // Transparent fill for click and hover inspection of any cell around Antarctica
      mapInstance.addLayer({
        id: 'full-h3-hit',
        type: 'fill',
        source: 'full-h3-source',
      ...
      // Hover H3 Cell: show H3 coordinates tooltip
      mapInstance.on('mousemove', 'full-h3-hit', (e) => {
        if (!e.features || e.features.length === 0) return;
        const feat = e.features[0];
        const props = feat.properties || {};
        const h3Id = props.id || props.h3_index || 'H3-CELL';
        
        mapInstance.getCanvas().style.cursor = 'pointer';
        setHoveredCellData({ ...props, displayId: h3Id });
      });

      mapInstance.on('mouseleave', 'full-h3-hit', () => {
        mapInstance.getCanvas().style.cursor = '';
        setHoveredCellData(null);
      });

      // Click H3 Cell: Select cell
      mapInstance.on('click', 'full-h3-hit', (e) => {
        if (!e.features || e.features.length === 0) return;
        const feat = e.features[0];
        const props = feat.properties || {};
        const cellId = props.id || props.h3_index || 'H3-CELL';
        ...
        if (mapInstance.getLayer('full-h3-selected-line')) {
          mapInstance.setFilter('full-h3-selected-line', ['==', ['get', 'id'], cellId]);
        }

        if (onInspectPoint && props.lat) {
          onInspectPoint([props.lat, props.lon]);
        }
      });
  ...
  useEffect(() => {
    if (!map.current || !map.current.isStyleLoaded()) return;
    if (map.current.getLayer('full-h3-lines')) {
      map.current.setLayoutProperty('full-h3-lines', 'visibility', showH3Grid ? 'visible' : 'none');
    }
  }, [showH3Grid]);
  ...
  useEffect(() => {
    ...
    if (map.current.getLayer('full-h3-selected-line')) {
      map.current.setFilter('full-h3-selected-line', ['==', ['get', 'id'], selId]);
    }
  }, [selectedH3Cell]);
    ...
    <div style={{ position: 'relative', width: '100%', height: '100%', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, position: 'relative', minHeight: '380px' }}>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <span style={{ fontSize: '11px', fontWeight: 800, fontFamily: 'var(--font-mono)', color: '#f8fafc' }}>
              NCPOR AMIP // CIRCUM-ANTARCTIC H3 GRID
            </span>
          ...
          <div style={{
            ...
            <span style={{ fontSize: '10px', color: '#cbd5e1', fontFamily: 'var(--font-mono)' }}>
              TIME: <strong style={{ color: '#38bdf8' }}>T+{sliderDay}d ({currentHz})</strong> | ACTIVE ROUTE: <strong style={{ color: STABLE_ROUTE_COLORS[activeRouteId] }}>{(selectedRoute?.objective || activeRouteId).toUpperCase()}</strong> | TRACKED BERGS: <strong style={{ color: '#f97316' }}>{icebergsList.length}</strong>
            </span>
          </div>
        </div>
        ...
        <div style={{
          ...
          >
            <Grid size={11} />
            <span>ALL H3 CELLS ({showH3Grid ? 'ON' : 'OFF'})</span>
          </button>
        ...
        {hoveredCellData && (
          <div style={{
            ...
            <div style={{
              ...
              <span>H3 CELL: <strong style={{ color: '#38bdf8' }}>{hoveredCellData.displayId}</strong></span>
              <span style={{ margin: '0 6px', color: '#64748b' }}>|</span>
              <span>COORDS: <strong style={{ color: '#cbd5e1' }}>{hoveredCellData.lat?.toFixed(2)}°S, {hoveredCellData.lon?.toFixed(2)}°E</strong></span>
            </div>
          </div>
        )}

        {/* ------------------------------------------------------------- */}
        {/* Route Selector & Individual Toggle Panel (Top-Right / Left) */}
        {/* ------------------------------------------------------------- */}
        <div style={{
          position: 'absolute',
          top: '50px',
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
              ...
              <span>Full H3 Mesh ({fullAntarcticaH3GeoJSON.features.length} cells)</span>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
...
print("Successfully wrote clean AntarcticMap.tsx with full circum-Antarctic H3 cells and route toggles.")
